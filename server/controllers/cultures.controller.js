...
export const createCulture = async (req, res) => {
    const { nom, variete, date_semis, date_recolte_prevue, statut, id_parcelle } = req.body
    try {
        const [result] = await db.query(`
            INSERT INTO culture (nom, variete, date_semis, date_recolte_prevue, statut, id_parcelle)
            VALUES (?, ?, ?, ?, ?, ?)    
        `, [nom, variete ?? null, date_semis, date_recolte_prevue ?? null, statut ?? 'en cours', id_parcelle])
...
export const updateCulture = async (req, res) => {
    const { nom, variete, date_semis, date_recolte_prevue, statut } = req.body

    try {
        await db.query(`
            UPDATE culture
            SET nom = ?, variete = ?, date_semis = ?, date_recolte_prevue = ?, statut = ? 
            WHERE id_culture = ?
        `, [nom, variete ?? null, date_semis, date_recolte_prevue ?? null, statut, req.params.id])
